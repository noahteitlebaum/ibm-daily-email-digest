...
def _recipients() -> list[str]:
    """Fallback recipients from ONLY_TO (single recipient variable)."""
    raw = os.getenv("ONLY_TO", "")
...
def send_digest(subject: str, html_body: str, text_body: str,
    ...
    if not recipients:
        raise RuntimeError(
            "No recipients. Either populate the team's recipients in "
            "config/teams.yaml, or set ONLY_TO to comma-separated addresses."
        )
