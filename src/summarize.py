...
def summarize(articles: list[Article], client: LLMClient,
              grounding: str | None = None) -> dict:
    """Return the structured digest dict (tldr + insights).

    grounding: pass an account-brief string to ground the insights; pass "" for
    teams that have no brief (news-only framing); None uses the default brief.
    """
    if not articles:
    ...
    prompt = USER_TEMPLATE.format(
        grounding=grounding or "(No account brief for these clients. Do not invent "
                  "buyers, dollar figures, or contacts; mark such fields 'to validate'.)",
        product_hierarchy=config.product_hierarchy_text(),
